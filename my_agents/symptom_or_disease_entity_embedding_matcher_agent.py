...
class SymptomOrDiseaseEntityEmbeddingMatcherAgent(Runnable):
    def invoke(self, agent_state: AgentState, config: dict = None) -> AgentState:
        ...
        if not entities:
            agent_state['symptom_or_disease_entity_match_results'] = []
            return agent_state
        index_path = Path(conf.FAISS_INDEX_BASE) / "disease_symptom_index.bin"
        meta_path = Path(conf.FAISS_INDEX_BASE) / "disease_symptom_meta.json"
        ...
        agent_state['symptom_or_disease_entity_match_results'] = results
        return agent_state
